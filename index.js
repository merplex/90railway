...
async function listSubReport(replyToken, type) {
    try {
        ...
        if (type === "PENDING") {
            ...
            rows = res.rows.map(r => ({ type: "box", layout: "horizontal", margin: "sm", alignItems: "center", spacing: "sm", contents: [{ type: "text", text: r.line_user_id.substring(0,6), size: "xxs", flex: 3, color: "#4267B2", action: { type: "message", label: r.line_user_id, text: `GET_HISTORY ${r.line_user_id}` } }, { type: "text", text: `+${r.points}p`, size: "xxs", flex: 2, color: "#00b900", weight: "bold" }, { type: "button", style: "primary", color: "#00b900", height: "sm", action: { type: "message", label: "✓", text: `APPROVE_ID ${r.id}` }, flex: 2 }, { type: "button", style: "primary", color: "#ff4b4b", height: "sm", action: { type: "message", label: "✗", text: `REJECT_ID ${r.id}` }, flex: 2 }] }));
        } else if (type === "EARNS") {
